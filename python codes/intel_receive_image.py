...
ref = s.getsockname()[0]
simage = socket.socket(socket.AF_INET, socket.SOCK_STREAM);
simage.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

simage.bind((ref, 9876));  # IMAGE SENDING PORT
    ...
    try:
        ...
        while (1):
            ...
            if (len(data) >= datasize):
                break;
        cobj.send("1".encode());
        frame = pickle.loads(data,encoding="latin1");
